...
_REMOTE_DEVICES_CACHE_MTIME: float = 0.0
...
def export_local_snapshot(device_id: str, device_name: str) -> dict[str, Any]:
    ...
    具备【增量吸收保护 (Merge-on-Export)】：
    自动读取已有历史快照做并集合并，即使本地原生 Agent 日志被清理或轮转，历史用量也永不丢失！
    """
    repo_dir = get_sync_repo_dir()
    ...
    for aid, adapter in ADAPTERS.items():
        ...
        try:
            ...
            for s in session_list:
                ...
                if sid:
                    seen_sids.add(sid)
                title = s.get("title") or titles.get(sid, "")
...
def invalidate_remote_cache() -> None:
    """失效异机内存缓存，促使下次读取重新加载"""
    global _REMOTE_DEVICES_CACHE, _REMOTE_DEVICES_CACHE_MTIME
    _REMOTE_DEVICES_CACHE = None
    _REMOTE_DEVICES_CACHE_MTIME = 0.0
...
def load_remote_devices_data() -> dict[str, Any]:
    ...
    try:
        current_mtime = os.path.getmtime(devices_dir)
    except OSError:
        current_mtime = 0.0

    if _REMOTE_DEVICES_CACHE is not None and current_mtime <= _REMOTE_DEVICES_CACHE_MTIME:
    ...
    try:
        ...
        _REMOTE_DEVICES_CACHE_MTIME = current_mtime
        return _REMOTE_DEVICES_CACHE
